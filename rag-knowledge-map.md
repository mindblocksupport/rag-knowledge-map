...
#### 0.1.7 一图胜千言 — 完整 RAG 数据流 (Hybrid 标准版)
- 离线 (一次, Index Build):
  - 文档 → Parser → Chunking → 分两路并行处理:
    - 路径 A: Embedder → (chunk_id, vector) → 向量库
    - 路径 B: 分词 + 倒排表 → (term, chunk_id, IDF, len) → 倒排索引库
  - 同时 (chunk_id + 原文 text) 入文档库
  - 三存储共享 chunk_id 作为连接键
- 在线 (每次 query, Hybrid Retrieval):
  - query 双路预处理:
    - 路径 A: query → Embedder (同一个) → query 向量
    - 路径 B: query → tokenizer (同一个) → query terms
  - 双路并行检索:
    - 路径 A: query 向量 → ANN 搜索向量库 → top-50 chunk_id + cosine
    - 路径 B: query terms → 倒排查询 → top-50 chunk_id + BM25 score
  - RRF 融合 → top-20
  - Reranker (Cross-Encoder) 精排 → top-5
  - 回查 Doc Store → top-5 原文 text
  - Prompt 拼接 (原文 text + query) → LLM → 答案 + 引用
- 三存储职责分工:
  - 向量库: 只做"找语义相似的 chunk_id" (Dense 召回)
  - 倒排索引库: 只做"找字面匹配的 chunk_id" (Sparse 召回, BM25 兜底)
  - 文档库: 只做"按 chunk_id 取原文" (供 LLM 阅读)
  - LLM: **喂给 LLM 的是原文 (text), 不是向量, 不是 score** — 向量和 score 只是检索阶段的中间产物, 找完就丢
- 三个 score 的归宿:
  - Cosine score (Dense): 用于 ANN 排序, 进 RRF 后丢弃
  - BM25 score (Sparse): 用于倒排排序, 进 RRF 后丢弃
  - Reranker score (Cross-Encoder): 用于最后精排, 切完 top-K 后丢弃
  - 三种 score 都不喂给 LLM, 只用于"挑出哪几条原文给 LLM"
...
### 8.5 7 种高级 RAG-Agent 模式 (每种完整流程详解)
...
#### 8.5.5 FRAG (Flexible RAG, 2025 推测, 业界尚无定论)

##### 核心思想 (注: 该术语在主流文献库未明确对应论文, 多家厂商内部用类似设计)
- 把 Naive RAG / GraphRAG / Self-RAG 三种放在一起, query 自适应分流
- 简单 query → Naive RAG (快+便宜)
- 中等 query → Advanced RAG (Reranker + HyDE)
- 复杂 query → GraphRAG / Self-RAG (慢+贵)

##### 完整执行流程
- 步 1: query 输入
- 步 2: Classifier (轻量模型) 判断复杂度: simple / medium / complex
- 步 3: 分流:
  - simple → Naive RAG (5 步标准流水)
  - medium → Advanced RAG (Hybrid + Reranker + HyDE)
  - complex → GraphRAG 或 Self-RAG
- 步 4: 各自走完整流程
- 步 5: 输出答案

##### 性能数字 (推测)
- 综合成本下降 40-60% (因为大部分 query 走 Naive)
- 复杂 query 质量保持 GraphRAG 水平

##### 真实采用 (推测)
- 大型企业搜索内部架构 (Glean / Notion)

#### 8.5.6 GraphIRAG (Graph + Iterative RAG, 2025 推测)

##### 核心思想 (注: 同 FRAG, 该术语未明确对应单篇论文, 但迭代式 GraphRAG 是业界趋势)
- GraphRAG 单次检索 → 多轮迭代检索
- 每轮根据已检索内容, 用信息增益准则决定下一轮检索什么
- 类似 Plan-and-Execute Agent 但聚焦在图上

##### 完整执行流程
- 步 1: query 输入
- 步 2: 第一轮 GraphRAG 检索 → top-K1 chunks
- 步 3: LLM 评估: 信息够答了吗?
  - 够 → 进步 6
  - 不够 → 生成下一轮检索 query (基于已知信息的 gap)
- 步 4: 第二轮检索 (新 query) → top-K2 chunks
- 步 5: 重复步 3-4, 最多 N=5 轮
- 步 6: 拼所有轮次 chunks → LLM 答

##### 信息增益停止准则
- 计算第 N 轮新 chunks 与已有 chunks 的 cosine 相似度
- 如新 chunks 与已有重合 > 80% (信息增益低), 停止
- 或答案 confidence > 0.9, 停止

##### 性能数字 (推测)
- 多跳推理 +25-40% accuracy vs 单次 GraphRAG
- 延迟 +3-5× (多轮检索)

#### 8.5.7 Adaptive RAG (Jeong et al. 2024, arXiv:2403.14403)
...
| 场景 | RAG 必要性 | 关键 Layer | Agent 必要性 |
|---|---|---|---|
| 法规合规 | 极高 | L1-L3 | 低 |
| 内部 KB | 极高 | L1-L4 | 中 |
| 客服 | 高 | L1-L5 | 高 |
| 代码 | 中 (Agentic) | L1, L5 | 极高 |
| 销售 | 中 | L1-L4 | 中 |
| 数据分析 | 中 | L4 (Text2SQL) | 中 |
| 多模态 | 高 | L1-L3 | 中 |
| 跨系统诊断 | 低 | L5 | 极高 |
...
- 80% 简单 query → 普通 RAG (单次 hybrid + rerank, $0.001 / 1-2s)
- 15% 中等查询 → 增强 RAG (HyDE / Multi-Query / Self-RAG, $0.005 / 2-3s)
- 5% 高价值查询 → Agent (多步规划 + 工具调用, $0.05-0.5 / 5-30s)
...
##### 完整高分答案
Agent 死循环是真实事故: 2024.11 某 SaaS 死循环 1 小时烧 $5000. 必须多道防线.
...
(3) budget cap per query:
- 单 query 成本上限 $1
- LLM 累计 cost 监控
- 超过强制停

(4) 死循环检测:
- 同 tool 重复 3 次 (同 tool name + 同参数) → 熔断
- 给 LLM 信号 "你已经试过这个工具 3 次, 试别的"
- 工具历史去重

(5) 告警:
- 单 query > $0.5 → 进 review 队列
- 单 query 步数 > max_steps × 0.8 → 异常告警
- 周度异常统计
...
工具支持:
- LangSmith / Phoenix / Langfuse 全链路追踪
- 实时看每步 LLM call + tool call
- 失败步骤标红
- Bad case 自动收集

真实事故复盘:
- 2024.11 某 SaaS
- Agent 调 get_order_status("12345"), 工具返回 timeout error
- LLM 决定重试 → 仍 timeout
- LLM 决定重试 → 仍 timeout
- ...循环 5000 次, 1 小时烧 $5000
- 修复: max_tool_calls_per_step + 同工具重复 3 次熔断 + budget cap
...
A: 看模式. 正常: 不同 tool 不同步骤前进. 死循环: 同 tool 同参数反复. 自动检测: 调用历史去重 + LLM 熵检测 (步骤间相似度高 → 死循环).

##### 第三轮追问 Q: 死循环熔断后怎么处理?
A: 三选: (1) 拒答 ("当前查询无法完成, 请重试或转人工") (2) Fallback 到普通 RAG (3) 告警 + 转人工. 不要让用户感知熔断 (用户体验最差).

##### 反例
- ❌ "信任 LLM 不限步" — 死循环必崩
- ❌ "死循环算 LLM 锅, 没办法" — 工程必须防
...
#### 20.8.1 三大杠杆
- per-user QPS 限制 (Free 1, Pro 10, Enterprise 100)
- per-tool retry 限制
- 异常 query 熔断 (同 user 同 query 5 分钟内重复 → block)

#### 20.8.2 路由优化
- 80% 简单 query 不走 Agent (普通 RAG 即可)
- 5% 高价值才上 Agent (Klarna 真实数字)

#### 20.8.3 模型选型
- Planner 用 Sonnet (准, 决策关键)
