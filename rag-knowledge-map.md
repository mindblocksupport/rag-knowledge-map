...
### 2.2 流程 1: 文档入库 (Ingestion 文档摄取) — 完整 6 步详解
...
##### 步 3: 数据处理 (Data Processing 清洗 + 治理)

###### 是什么
- Parse 后拿到的是"原始纯文本", 但生产级 RAG 不能直接进 Chunking, 必须先做一系列治理
- 真实数据 80% 时间在做这一步 (业界共识 "Garbage In, Garbage Out")
- 跳过这步 = LLM 答案必然差, 检索召回也差

###### 6 道数据治理子环节 (生产必做)

**(1) 去噪 (Cleaning)**
- 去 HTML tag / 广告 / 导航 / 页眉页脚 / 水印 / 多余空白
- 去重复段落 (同一份 PDF 每页都有版权声明)
- 工具: BeautifulSoup / readability-lxml / 正则
- 真实影响: 去噪前 chunk 30% 是噪声 → 检索精度跌 15%

**(2) 格式化 (Normalization)**
- 统一编码 (UTF-8 强制), 处理 BOM / 乱码
- 统一换行 (\r\n → \n)
- 全角/半角统一 (中文逗号, vs 英文逗号 ,)
- Unicode 归一化 (NFKC, 处理同形异码)
- 工具: ftfy / unicodedata
- 反模式: 不格式化 → query "用户" 命中不了 chunk "用 户" (有空格的同形)

**(3) 去重 (Deduplication)**
- 文档级去重: SHA256 hash 完全相同丢弃 (e.g. 同 PDF 上传 2 次)
- 段落级近似去重: MinHash + LSH (Jaccard ≥ 0.85 视为重复)
- 跨文档段落去重: 法律条款 / 免责声明 在 100 个合同里重复, 只留 1 份
- 工具: datasketch (MinHash) / Apache Spark MinHashLSH
- 真实影响: 去重前 100 万文档实际唯一内容 60 万, 浪费 40% 存储 + 检索召回重复结果用户骂

**(4) PII 检测与脱敏 (Privacy)**
- 检测: 身份证 / 手机 / 邮箱 / 信用卡 / SSN / 银行账号 / 地址
- 工具: Microsoft Presidio (规则 + NER 双引擎) / spaCy NER + 自定义正则
- 中文 NER 需 fine-tune: 通用模型对中文身份证 / 中文手机识别率 50-70%, fine-tune 后 95%+
- 处理策略 3 选 1:
  - 脱敏: 张三 → [PERSON_001], 13800138000 → [PHONE_001]
  - 删除: 直接丢弃含 PII 的 chunk (合规严的场景)
  - 标记不上索引: 入库但加 sensitivity=high, 检索时按用户权限过滤
- 真实事故: §13.27 Microsoft Recall — 截屏未做 PII 脱敏直接落盘, 银行密码 / 私聊 / 信用卡裸奔

**(5) 质量评分 + 过滤 (Quality Gating)**
- 检测低质内容: 全是表情符号 / 乱码 / 测试文档 / 内容过短 (<50 字) / 重复字符
- 评分方法 2 种:
  - 启发式规则: text length / unique tokens ratio / language detection / 标点比例
  - LLM-as-judge: Haiku/Flash 调一次, 给 1-5 分 (3 维度: 信息密度 / 可读性 / 完整性), 阈值过滤
- 成本: Haiku $0.001/文档, 100 万文档 $1000
- ROI: 过滤掉低质 5-15%, 召回率提升 10-20%

**(6) 分类打标 + 版本管理 (Classification + Versioning)**
- topic / language / department / sensitivity 自动打标 (LLM 或 fine-tuned 分类器)
- 版本管理: 同 doc 多版本 (canonical_id + version), 新版进来旧版归档不删 (法规 / 审计需求)
- 时效性: 抓取 created_at / updated_at, 检索时支持 recency_decay (近期文档加权)
- 真实场景: §13.13 OpenAI v2→v3 embedding 集体迁移 — 必须有 canonical_id 才能双写过渡

###### 处理顺序 (有先后)

按依赖顺序: 去噪 → 格式化 → 去重 → PII → 质量 → 分类
- 先去噪格式化 (干净的文本才能正确做后续)
- 再去重 (避免对重复文档做后面的昂贵处理)
- 再 PII (去重后再脱敏, 减少 50%+ 调用量)
- 最后质量 + 分类 (只对值得保留的文档做)

###### 工程实现建议 (生产架构)

- 异步队列: Kafka / RabbitMQ + Celery worker, 不阻塞前端上传
- 失败重试 + DLQ (死信队列): 单文档处理失败不能堵塞全链
- 监控指标: dedup_skip_ratio / pii_detected_ratio / quality_pass_ratio / dlq_size
- 详见 §4 L1 数据治理 完整流程

###### 跳过这步的真实代价 (3 个事故)

- ❌ 不去重: 100 万文档实际唯一 60 万 → 检索召回重复结果, 用户体验差
- ❌ 不做 PII: §13.27 Microsoft Recall 隐私事故, 全网骂上热搜
- ❌ 不质量过滤: 测试文档 / 乱码进 KB, 检索召回时 LLM 看到一堆"asdfasdf"无法回答

###### 关键决策

- 内部 KB (低敏感): 去噪 + 格式化 + 去重 + 质量过滤 (跳过严格 PII)
- 客户场景 (高敏感): 6 道全做 (PII 严格脱敏 + 双重 LLM-as-judge)
- 极致合规 (医疗/金融): 6 道全做 + 人工 review 5% 抽样

###### 常见误区

- ❌ "Parse 完就 Chunk" — 跳过数据处理 = Garbage In Garbage Out, 工业 RAG 必栽
- ❌ "数据处理是一次性的" — 每次入库都要做, 不是部署时跑一次
- ❌ "PII 脱敏放在检索时做" — 太晚, 必须入库前; 检索时做泄露已发生
- ✅ "异步队列 + 6 道治理 + DLQ + 监控告警, 详见 §4 L1 数据治理"

##### 步 4: 文档切块 (Chunking 分块)
...
> ⚠️ 注: 上表省略了"步 3 数据处理"成本. 真实生产 100 万文档完整 Ingest 数据处理 (去噪 + 去重 + PII + 质量评分 + 分类) 额外占 **3-5 小时 + ~$1.5K** (Haiku 质量评分 100 万 × $0.001 ≈ $1K + 分类打标 $300 + CPU $200), 占总成本 35-40%. 跳过会导致召回塌 + 合规风险, 详见 §2.2.3 步 3 + §4 L1 数据治理.
